...
| H-3 (significant degradation: mandatory handoff condition) | **Autonomous hold** | Platform holds current state; suspends all new mission or action initiation; continues existing committed actions only if pre-authorized as safe-to-complete; awaits confirmed alternate or safe-state transition |
